function [cExt, cSpec] = func_signalcut_yaxis (path, filename)
...
hp_data = highpass(data, 150, rate);               % highpass filtering을 진행
...
target_y = hp_data(:, 2);             % yout_scaled
target_y = target_y - mean(target_y);     % 각 요소-전체의 평균(편차)
target_y = target_y ./ max(target_y);     % 편차 / 최대편차
...
for cnt = 1:numVib
    cExt(cnt, :) = target_y(locs(count) + rate * 1.4 + (cnt-1) * rate * (vibLength + 1) + (1:coarseInterval)); 
